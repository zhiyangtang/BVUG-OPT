...
void STPG::init() {
    ...
    polarityPointer = new EdgeID[VN];
    inQueue = new bool[VN];
    
    dropflag = new VertexID[VN]();
    tpg_graph = new PerEdge[EN];
    newtpg_graph = new PerEdge[EN];
...
void STPG::answerAllQueries(vector<PerQuery>& queries) {
    ...
    for (PerQuery& query : queries) {
        ...
        resultEnd = executeQuery(query.source, query.target, query.star, query.end+6);
        ...
        // batch update
        for (int ba = 1; ba <= 8; ++ba) {
            // Problem 2
            if (tfend <= query.end + 6 - ba) {
                #ifdef WRITE_ANSWERS
                    resultFile << " - " << endl;
                #endif
                #ifdef WRITE_STATISTICS
            ...
            else if (tmfend > query.end + 6 - ba) {
                #ifdef WRITE_ANSWERS
                    resultFile << " - " << endl;
                #endif
                #ifdef WRITE_STATISTICS
                    statisticsFile << " - " << "," << "0" << "," << "0" << "," << "0" << endl;
                #endif
            }
            else {
                batchAnswer(query.end + 6 - ba);
            }

            // Problem 3
            // if (tfbegin >= query.star + ba) {
            //     #ifdef WRITE_ANSWERS
            //         resultFile << " - " << endl;
            //     #endif
            //     #ifdef WRITE_STATISTICS
            ...
            //     #endif
            // }
            // else if (tmfbegin < query.star + ba) {
            //     #ifdef WRITE_ANSWERS
            //         resultFile << " - " << endl;
            //     #endif
            //     #ifdef WRITE_STATISTICS
...
void STPG::batchAnswer(Time t_fend) {
    
    resultEnd = 0, upperEnd = 0, newtpgEnd = 0, edgesForVerificationEnd = 0;
    memset(backwardFlag, offset, sizeof(int)*VN);
    memset(hasVerified, 0, sizeof(EdgeID)*EN);
    memset(arrivals, 0, sizeof(VertexID)*VN);
    delete[] new_sortInEdges;
    delete[] new_sortOutEdges;

    VertexID* update = new VertexID[VN]();
    VertexID* curforwardEVLen = new VertexID[VN]();
    ...
    for (int i = tpgEnd - 1; i >= 0; --i) {
        ...
        if (time <= t_fend) {
            if (backwardFlag[from] == offset + 2 && to == t) {
                results[resultEnd++] = tpg_graph[i].edgeId;
                newtpg_graph[newtpgEnd++] = tpg_graph[i];
                ...
                if (to == t) {
                    ...
                    if (from != s) {
                        ...
                        if (!arrivals[from])
                            arrivals[from] = time;
                        if (!update[from]) {
                            latestTime[from] = time + offset;
                            backwardEV[backwardLocator[from]].vertices = new VertexID[1]{from};
                            backwardEV[backwardLocator[from]].time = time;
                            ...
                            backwardEV[backwardEVLen[from] - offset].vertices = new VertexID[1]{from};
                            backwardEV[backwardEVLen[from] - offset].time = time;
                            backwardEV[backwardEVLen[from] - offset].size = 1;
                        }
                    }
                }
                else {
                    if (update[to] && latestTime[to] > time + offset) {
                        ...
                        if (from == s) {
                            ...
                            if (backwardFlag[from] != offset + 1 && backwardFlag[from] != offset + 2) {
                            ...
                            if (!curforwardEVLen[from])
                                curforwardEVLen[from] = forwardEVLen[from];
                            while (forwardEV[curforwardEVLen[from] - offset].time >= time)
                                curforwardEVLen[from] -= 1;

                            VertexID *forward, *backward;
                            int forsize, backsize;

                            forward = forwardEV[curforwardEVLen[from] - offset].vertices;
                            forsize = forwardEV[curforwardEVLen[from] - offset].size;

                            backward = backwardEV[locator].vertices;
                            backsize = backwardEV[locator].size;
                            ...
                            if (end - tmp == 0) {
                                upper_bound_graph[upperEnd++] = tpg_graph[i];
                                if (forwardFlag[from] == offset + 2 && backwardFlag[to] == offset + 2 && departures[from] < time && arrivals[to] > time) {
                                    results[resultEnd++] = tpg_graph[i].edgeId;
                                    hasVerified[tpg_graph[i].edgeId] = offset + 1;
                                }
                                else if (forwardFlag[from] == offset + 2 && departures[from] < time) {
                                    results[resultEnd++] = tpg_graph[i].edgeId;
                                    hasVerified[tpg_graph[i].edgeId] = offset + 1;
                                }
                                else if (backwardFlag[to] == offset + 2 && arrivals[to] > time) {
                                    results[resultEnd++] = tpg_graph[i].edgeId;
                                    hasVerified[tpg_graph[i].edgeId] = offset + 1;
                                }
                                else {
                        ...
                        if (forwardEVLen[to] - offset != forwardLocator[to]) {
                            if (forwardEV[forwardEVLen[to] - offset].time == time) {
                                forwardEVLen[to] -= 1;
                                if (forwardEV[forwardEVLen[to] - offset].size != 1)
                                    forwardFlag[to] = offset;
                            }
                        }
                    }
                }
    ...
    delete[] update;
    delete[] curforwardEVLen;

    std::sort(newtpg_graph, newtpg_graph + newtpgEnd, sortByEdgeTime);

    PerEdge *tg_tmp = tpg_graph;
    tpg_graph = newtpg_graph;
    newtpg_graph = tg_tmp;
    tpgEnd = newtpgEnd;
    ...
    #endif
    #ifdef WRITE_STATISTICS
        statisticsFile << " - " << "," << tpgEnd << "," << upperEnd << "," << resultEnd << endl;
    #endif
}


void STPG::batchAnswer2(Time t_fbegin) {
    
    resultEnd = 0, upperEnd = 0, newtpgEnd = 0, edgesForVerificationEnd = 0;
    memset(forwardFlag, offset, sizeof(int)*VN);
    memset(hasVerified, 0, sizeof(EdgeID)*EN);
    ...
    VertexID* update = new VertexID[VN]();
    VertexID* curbackwardEVLen = new VertexID[VN]();
    ...
    for (int i = 0; i < tpgEnd; ++i) {
        ...
        if (time >= t_fbegin) {
            if (forwardFlag[to] == offset + 2 && from == s) {
                results[resultEnd++] = tpg_graph[i].edgeId;
                newtpg_graph[newtpgEnd++] = tpg_graph[i];
                upper_bound_graph[upperEnd++] = tpg_graph[i];
                hasVerified[tpg_graph[i].edgeId] = offset + 1;
                ...
                if (from == s) {
                    if (!update[from]) {
                        earliestTime[from] = offset + t_fbegin - 1;
                        tfbegin = time;
                        update[from] = 1;
                    }
                    ...
                    if (to != t) {
                        ...
                        if (!departures[to])
                            departures[to] = time;
                        if (!update[to]) {
                            earliestTime[to] = time + offset;
                            forwardEV[forwardLocator[to]].vertices = new VertexID[1]{to};
                            forwardEV[forwardLocator[to]].time = time;
                            ...
                            forwardEVLen[to] += 1;
                            forwardEV[forwardEVLen[to] - offset].vertices = new VertexID[1]{to};
                            forwardEV[forwardEVLen[to] - offset].time = time;
                            forwardEV[forwardEVLen[to] - offset].size = 1;
                        }
                    }
                }
                else {
                    if (update[from] && earliestTime[from] < time + offset) {
                        newtpg_graph[newtpgEnd++] = tpg_graph[i];
                        if (to == t) {
                            upper_bound_graph[upperEnd++] = tpg_graph[i];
                            hasVerified[tpg_graph[i].edgeId] = offset + 1;
                            ...
                            while (time == forwardEV[locator].time)
                                locator--;
                            if (forwardFlag[to] != offset + 1 && forwardFlag[to] != offset + 2) {
                            ...
                            if (!curbackwardEVLen[to])
                                curbackwardEVLen[to] = backwardEVLen[to];
                            while (backwardEV[curbackwardEVLen[to] - offset].time <= time)
                                curbackwardEVLen[to] -= 1;

                            VertexID *forward, *backward;
                            int forsize, backsize;

                            forward = forwardEV[locator].vertices;
                            forsize = forwardEV[locator].size;

                            backward = backwardEV[curbackwardEVLen[to] - offset].vertices;
                            backsize = backwardEV[curbackwardEVLen[to] - offset].size;
                       ...
                       if (backwardEVLen[from] - offset != backwardLocator[from]) {
                            if (backwardEV[backwardEVLen[from] - offset].time == time) {
                                backwardEVLen[from] -= 1;
                                if (backwardEV[backwardEVLen[from] - offset].size != 1)
                                    backwardFlag[from] = offset;
                            }
                       } 
                    }
                }
...
EdgeID STPG::executeQuery(VertexID source, VertexID target, Time begin, Time end) {
    ...
    earliestTimeBFS();
    tmfend = earliestTime[t] - offset;
    latestTimeBFS();
    tmfbegin = latestTime[s] - offset;
    prunedVertices();
    prunedEdges();
...
void STPG::earliestTimeBFS() {
    ...
    memset(inQueue, 0, sizeof(bool)*VN);
    inQueue[s] = true;
    for (VertexID u = 0; u < VN; u++) {
        polarityPointer[u] = outNeighborsLocator[u + 1];
    }
    ...
    while (forwardFrontierEnd > 0) {
        ...
        for (VertexID i = 0; i < forwardFrontierEnd; i++) {
            ...
            EdgeID& pointer = polarityPointer[u];
            while (pointer > outNeighborsLocator[u]) {
                EdgeID edgeLocator = pointer - 1;
                ...
                if (v == s || (earliestTime[v] > offset && time + offset >= earliestTime[v])) {
                    continue;
                }

                earliestTime[v] = time + offset;
                if (time != t_end && v != t && !inQueue[v]) {
...
void STPG::latestTimeBFS() {
    ...
    inQueue[t] = true;
    for (VertexID u = 0; u < VN; u++) {
        polarityPointer[u] = inNeighborsLocator[u];
    }
    ...
    while(backwardFrontierEnd > 0) {
        ...
        for (VertexID i = 0; i < backwardFrontierEnd; i++) {
            ...
            EdgeID& pointer = polarityPointer[u];
            while (pointer < inNeighborsLocator[u + 1]) {
                EdgeID edgeLocator = pointer;
                ...
                if (time > t_end) {
                    pointer = inNeighborsLocator[u + 1];
                    break;
                }
                ...
                if (v == t || (latestTime[v] > offset && time + offset <= latestTime[v])) {
                    continue;
                }

                latestTime[v] = time + offset;
                if (time != t_begin && v != s && !inQueue[v]) {
                    nextFrontier[nextFrontierEnd++] = v;
                    inQueue[v] = true;
                }
            }
...
void STPG::cleanUp() {
    ...
    double STPG::getCurrentSpaceCost() {
        ...
        spaceCost += sizeof(EdgeID)*VN;                         // polarityPointer
        spaceCost += sizeof(bool)*VN;                           // inQueue
